...
void DStre(S_account A[], int N) {
	...
	if (sophieu == 0) { gotoxy(60, 8);cout << "dummy";_getch(); }
		...
		while (thoat) {
			...
			gotoxy(31, 8 + cv);cout << str_to_name(ND[A[PHIEU[cv + bd].userid - 1].idu - 1].hovaten);
...
void suanguoidung(S_account A[], int &N) {
	...
	while (thoat1) {
		...
		if ((cvo == 0) && c == 13) {
			...
			kaguroo(ND1, sond1, ND, sond, fstring);//herebitch
			sotrang = sond / MAX_cot + 1;
			for (int i = sond;i<sotrang*MAX_cot;i++) {
				ND[i].id = -1;
			}
			...
			if (sond == 0) { gotoxy(60, 8);cout << "dummy";_getch(); }
				...
				while (thoat) {
					...
					if (c == 13) {
						...
						textcolor(240);
						gotoxy(55, 33);cout << str_to_name(ND[bd + cv].hovaten);
...
void inDS4(S_user B[], S_account D[], int pos, int n) {
	...
	for (int i = pos;i<n + pos;i++) {
		...
		gotoxy(74, 8 + i - pos);cout << str_to_name(B[D[i].idu - 1].hovaten);
	}
	textcolor(240);
};
